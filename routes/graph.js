...
async function graph() {
  ...
  nodeProcess(nodes, edges, validNodes, validChannels, txc)
  .then(async () => {
    return edgeProcess(edges, txc);
  })
  .then(async () => {
    return channelNodeCleanup(validChannels, txc);
  })
  .then(async () => {
    return nodeNodeCleanup(validNodes, txc);
  })
  .then(async () => {
    await txc.commit()
    console.timeEnd('ADD')
...
function nodeProcess(nodes, edges, validNodes, validChannels, txc) {
  let nodePromises = [];
...
function edgeProcess(edges, txc) {
  let edgePromises = [];
  for (let i = 0; i < edges.length; i++) {
    let edge = edges[i];
...
async function channelNodeCleanup(validChannels, txc) {
  let channelCleanupPromises = [];
      ...
      for (let i = 0; i < existingChannelIDs.records.length; i++) {
        ...
        if (!validChannels.has(existingChannelID)) {
          channelCleanupPromises.push(txc.run(
      ...
      return Promise.all(channelCleanupPromises);
}

async function nodeNodeCleanup(validNodes, txc) {
  const existingPubKeys = await txc.run(
    `MATCH (n:Node) RETURN n.pubKey AS pubKey`
  );

  let nodeCleanupPromises = [];
  for (let i = 0; i < existingPubKeys.records.length; i++) {
    ...
    if (!validNodes.has(existingPubKey)) {
      nodeCleanupPromises.push(txc.run(
  ...
  return Promise.all(nodeCleanupPromises);
}
